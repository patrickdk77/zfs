...
{
	return (secpolicy_vnode_setids_setgids(CRED(),
	    KGID_TO_SGID(ip->i_gid), idmap, zfs_i_user_ns(ip)) == 0);
}

int
zpl_set_posix_acl(zidmap_t *idmap, struct inode *ip,
    struct posix_acl *acl, int type, boolean_t check_setgid)
{
...
zpl_init_acl(struct inode *ip, struct inode *dir)
{
	...
	if (acl) {
		...
		if (S_ISDIR(ip->i_mode)) {
			error = zpl_set_posix_acl(zfs_init_idmap, ip,
			    acl, ACL_TYPE_DEFAULT, B_FALSE);
			if (error)
				goto out;
		}
		...
		if (error >= 0) {
			...
			zfs_mark_inode_dirty(ip);
			if (error > 0) {
				error = zpl_set_posix_acl(
				    zfs_init_idmap, ip, acl,
				    ACL_TYPE_ACCESS, B_FALSE);
			}
		}
	}
out:
...
zpl_chmod_acl(struct inode *ip)
{
	...
	if (!error)
		error = zpl_set_posix_acl(zfs_init_idmap, ip, acl,
		    ACL_TYPE_ACCESS, B_FALSE);
    ...
    const void *value, size_t size, int flags)
{
	...
	error = zpl_set_posix_acl(idmap, ip, acl, type, B_TRUE);
	zpl_posix_acl_release(acl);
